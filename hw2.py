...
def remove_stopwords_doc(doc: Document):
    return Document(doc.doc_id, *[[word for word in sec if word not in stopwords and word not in string.punctuation]
        for sec in doc.sections()])
...
def compute_tfidf(doc, doc_freqs, weights, n):
    ...
    for word in vec:
        if word in doc_freqs:
            result[word] = (vec[word] *  math.log2((n/doc_freqs[word])))
    return result
...
def jaccard_sim(x, y):
    num = dictdot(x, y)
    result = (sum(list(x.values())) + sum(list(y.values())) - num)
    result += 0.000001
    if num == 0:
        return 0
    return (num) / result
...
def interpolate(x1, y1, x2, y2, x):
    ...
    b = y1 - m * x1
    return m * x + b
def sum_with_none_check(iterable):
  sum = 0
...
def precision_at(recall: float, results: List[int], relevant: List[int]) -> float:
    ...
    '''
    #precision = (relevant documents and retrieved documents)/ retieved documents
    if not results:
        return 0
    list_of_recalls = []
    list_of_precisions = []
    ...
    for i, res in enumerate(results):
        if res in relevant:
            ...
            precision = relevant_intersection_retrieved / (i+1)
            recall_counter = relevant_intersection_retrieved / len(relevant)
            list_of_recalls.append(recall_counter)
            list_of_precisions.append(precision)

  
    if not list_of_recalls or not list_of_precisions:
        return 0.0
        # print(recall)
    if recall not in list_of_recalls:
       
        for i in range(1, len(list_of_recalls)):
            if recall >= list_of_recalls[i-1] and recall < list_of_recalls[i]:
                return interpolate(list_of_recalls[i-1], list_of_recalls[i-1], list_of_recalls[i], list_of_recalls[i], recall)
        return 0
    return list_of_precisions[list_of_recalls.index(recall)]
...
def mean_precision2(results, relevant):
    prec = []
    for i in range(1,11):
        prec.append(precision_at(i/10, results, relevant)) 
    return (1/10) * sum_with_none_check(prec)
...
def norm_recall(results, relevant):
    ...
    if not results or not relevant:
        return 0
    for i, doc_id in enumerate(relevant):
        rank_list.append(results.index(doc_id))
        rank_counter += i
    return 1 - ((sum(rank_list) - rank_counter)/(len(relevant) * (len(results) * len(relevant))))
...
def norm_precision(results, relevant):
    ...
    if not results or not relevant:
        return 0
    for i, doc_id in enumerate(relevant):
        rank_list.append(math.log2(results[doc_id]))
        rank_counter += i

    return 1 - ((sum(rank_list) - rank_counter)/(len(results)*math.log2(len(results))  -  (len(results) - len(relevant))* math.log2((len(results) - len(relevant)))  -  (len(relevant)*math.log2(len(relevant)))))


### Extensions

# TODO: put any extensions here
def relevance_feedback(query, doc_freqs):
...
# The system displays a revised set of retrieval results.
    ...
    # computing a new query centroid vector from a weighted linear combination
    # o the original query, the relevant-labelled documents and the
    # irrelevant-labelled documents
    print(doc_freqs)
    return 0
...
#   abstract: ['what', 'articles', 'exist', 'which', 'deal', 'with', 'tss', '(', 'time', 'sharing', 'system', ')', ',', 'an', 'operating', 'system', 'for', 'ibm', 'computers', '?'],
        author = input("Query: enter an author: ")
        title = input("Query: enter a title: ")
        keyword = input("Query: enter keywords: ")
        abstract = input("Query: enter in an abstract: ")
        queries = [Document(
             1,
           author.split(),
            title.split(),
    ...
    for term, stem, removestop, sim, term_weights in itertools.product(*permutations):
        ...
        metrics = []
        # for doc__id in [239, 1236, 2740]:
        #     query_vec = doc_vectors[doc__id-1]
        #     results_with_score = [(doc_id + 1, sim_funcs[sim](query_vec, doc_vec))
        #                 for doc_id, doc_vec in enumerate(doc_vectors)]
        #     results_with_score = sorted(results_with_score, key=lambda x: -x[1])
        #     results = [x[0] for x in results_with_score]
        #     print(f'At Document ID of : {doc__id}')
        #     for rank, (id,score) in enumerate(results_with_score[:20], 1):
        #         print(f'Rank: {rank} and Doc id: {id} and Title: {docs[id - 1].title} and score of similarity: {score}')
        #         #print(query_vec)
        #         #print(f'doc_id: {doc_id} and title: {docs[doc_id - 1].title} and score of similarity: {score}')
        #         # print('Score:', score)
        #         # print(docs[doc_id - 1])
        #     print()
        ...
        for query in processed_queries:
         ...
         #   relevance_feed = relevance_feedback(query_vec.values(), doc_freqs.values())
            
            #relevant_feed = relevance_feedback(query_vec, doc_freqs)
